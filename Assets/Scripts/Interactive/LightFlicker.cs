...
public class LightFlicker : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (lightManager)
        {
            switchID = lightManager.switchID;
        }
        ...
        if(tag == "AtkLight")
        {
            EventCenter.AddListener<Transform>(EventType.LightFlickerOn, FlickerOn); // 监听灯芯是否要开
            EventCenter.AddListener<Transform>(EventType.LightFlickerOff, FlickerOff);// 监听灯芯是否要熄灭
        }
    }

    void FlickerOn(Transform thisSwitch)
    {
        if(switchID == thisSwitch)
        {
            GetComponent<LightManager>().isSwitch = true;
            //lightIntensityMin = 1.5f;
            //lightIntensityMax = 3.5f;
            //GetComponent<BoxCollider>().enabled = true;
            //flickeringLight.enabled = true;
        }
    }
    void FlickerOff(Transform thisSwitch)
    {
        if (switchID == thisSwitch)
        {
            GetComponent<LightManager>().isSwitch = true;
